...
def check_git_info(path="."):
    ...
    try:
        ...
        # --- 修改开始：增加对 remotes 的安全检查 ---
        remote = None
        if hasattr(repo, 'remotes') and len(repo.remotes) > 0:
            try:
                # 优先尝试获取 origin，如果没有则获取第一个 remote
        ...
        except (TypeError, ValueError):  # not on any branch or detached HEAD
            branch = None
        
        return {"remote": remote, "branch": branch, "commit": commit}
    except (git.exc.InvalidGitRepositoryError, Exception):  # path is not a git dir or other git errors
